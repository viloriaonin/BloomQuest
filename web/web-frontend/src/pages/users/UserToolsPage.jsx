...
const UserToolsPage = ({ section }) => {
  ...
  const permanentlyDeleteSelected = async () => {
    ...
    if (!selectedItems.length || !window.confirm(`Permanently delete ${selectedItems.length} selected item${selectedItems.length === 1 ? "" : "s"}?`)) return;
  ...
  const deleteDownload = async (item) => {
    ...
    setSelectedDownloadIds((current) => current.filter((id) => id !== item.id));
  };

  const deleteSelectedDownloads = async () => {
    const selectedItems = downloads.filter((item) => selectedDownloadIds.includes(item.id));
    if (!selectedItems.length) return;
    const confirmed = window.confirm(`Delete ${selectedItems.length} selected downloaded file${selectedItems.length === 1 ? "" : "s"}?`);
    if (!confirmed) return;
